...
bool initDLSS() {
	...
	//UPSCALE_SCALE = 1.f;// static_cast<float>(pOutRenderOptimalHeight) / static_cast<float>(InUserSelectedHeight);
	...
	//DlssCreateParams.Feature.InWidth = WIDTH * UPSCALE_SCALE;
	//DlssCreateParams.Feature.InHeight = HEIGHT * UPSCALE_SCALE;
...
void RenderDLSS(VkCommandBuffer commandBuffer, uint32_t imageIndex, float sharpness) {
	...
	//evalParams.InRenderSubrectDimensions.Width = static_cast<unsigned int>(WIDTH * UPSCALE_SCALE);
	//evalParams.InRenderSubrectDimensions.Height = static_cast<unsigned int>(HEIGHT * UPSCALE_SCALE);
